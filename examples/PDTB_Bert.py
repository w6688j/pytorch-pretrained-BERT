...
class SimProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the train set."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "train.raw.txt")), "train")

    def get_dev_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the dev set."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "dev.raw.txt")), "dev")

    def get_test_examples(self, data_dir):
        """Gets a collection of `InputExample`s for prediction."""
        return self._create_examples(
            self._read_txt(os.path.join(data_dir, "test.raw.txt")), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line_split[1]
            text_b = line_split[2]
            label = self._laber_to_id(line_split[0])
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples

    def _laber_to_id(self, label):
        label_dict = {
            "Comparison": "0",
            "Contingency": "1",
            "Temporal": "2",
